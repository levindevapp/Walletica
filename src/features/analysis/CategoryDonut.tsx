...
import type { CategoryAggregate } from '../../types/api'

type CategoryDonutProps = { compact?: boolean; data: CategoryAggregate[] }

export function CategoryDonut({ compact = false, data }: CategoryDonutProps) {
    ...
    <div className={`grid items-center gap-4 ${compact ? 'md:grid-cols-[180px_1fr]' : 'md:grid-cols-[220px_1fr]'}`}>
      <div className={compact ? 'h-44' : 'h-52'}>
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={data} dataKey="amount" nameKey="name" innerRadius="58%" outerRadius="82%" paddingAngle={1}>
      ...
      <div className="space-y-2">
        {data.map((category) => (
          <div key={category.name} className="grid grid-cols-[10px_1fr_auto_auto] items-center gap-2 text-xs">
            ...
            <span className="text-slate-400">{total > 0 ? Math.round(category.amount / total * 100) : 0}%</span>
